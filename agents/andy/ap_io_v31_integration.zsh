...
# Purpose: Handle AP/IO v3.1 events for Andy (dev agent)
...
STATUS_FILE="$REPO_ROOT/agents/andy/status.json"
...
case "$EVENT_TYPE" in
  task_start)
    ...
    if [ -f "$WRITER" ]; then
      "$WRITER" andy info "$TASK_ID" "andy" "Dev task started: $TASK_ID" "{\"status\":\"acknowledged\",\"priority\":\"$PRIORITY\"}" >/dev/null 2>&1 || true
    fi
    ;;
    
  task_result)
    # Update Andy status to idle
    if [ -f "$STATUS_FILE" ]; then
      STATUS_DATA=$(cat "$STATUS_FILE")
  ...
  routing_request)
    ...
    echo "📨 Andy received routing request for: $TARGETS" >&2
    ;;
    
  *)
    echo "ℹ️  Andy received event: $EVENT_TYPE" >&2
    ;;
esac
