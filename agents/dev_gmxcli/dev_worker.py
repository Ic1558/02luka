"""
Dev GMXCLI Worker with direct-write capability via shared policy and Gemini CLI backend.
"""
...
class DevGMXCLIWorker:
    def __init__(self, backend: Optional[ReasonerBackend] = None):
        self.backend = backend or GeminiCLIBackend()
    ...
    def reason(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """
        Run reasoning via Gemini CLI backend. If a plan is provided, return it directly.
        """
        if "plan" in task:
            return task["plan"]
        if "patches" in task:
...
__all__ = ["DevGMXCLIWorker", "check_write_allowed", "apply_patch"]
