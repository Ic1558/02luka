 ...
 **Confirmed Finding:** The `work_notes.jsonl` system is **silently discarding old entries**, violating its documented "append-only journal"
 invariant.

 **Root Cause:** `bridge/lac/writer.py` implements a **rolling window** (WORK_NOTES_MAX = 200) that truncates entries, contradicting the
 "append-only" design promise.
