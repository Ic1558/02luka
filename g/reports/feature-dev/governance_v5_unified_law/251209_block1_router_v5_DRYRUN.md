...
bridge/
└── core/
    ├── router_v5.py          # Main router module (THIS BLOCK)
    ├── router_v5_config.yaml # Zone patterns & Mission Scope
    └── __init__.py           # Module exports
```

---

## 🎯 Router v5 Core Implementation

### Complete Python Module: `bridge/core/router_v5.py`
...
def resolve_world(trigger: str, context: Optional[Dict] = None) -> World:
    ...
    # Default: if context has WO id, assume BACKGROUND
    if context and context.get("wo_id"):
        return "BACKGROUND"
    
    # Fallback: assume CLI (safer default)
    return "CLI"
...
## 📝 Configuration File: `bridge/core/router_v5_config.yaml`

```yaml
# Router v5 Configuration
# Aligned with GOVERNANCE_UNIFIED_v5.md

# Zone Patterns (from Section 3.1)
zones:
  danger:
    - "^/$"
    - "^/System/"
...
# Mission Scope (from Section 2.4)
mission_scope:
  whitelist:
    - "^bridge/templates/"
    - "^g/reports/"
...
```python
# Example 1: CLI + OPEN Zone → FAST Lane
decision = route(
    trigger="cursor",
    actor="CLS",
...
# Result: zone=OPEN, lane=FAST, primary_writer=CLS

# Example 2: CLI + LOCKED Zone → WARN Lane
decision = route(
    trigger="cursor",
    actor="CLS",
    path="core/router.py",
...
# Result: zone=LOCKED, lane=WARN, primary_writer=CLS, auto_approve_allowed=False

# Example 3: CLI + LOCKED Zone (Mission Scope) → WARN Lane + Auto-approve
decision = route(
    trigger="cursor",
    actor="CLS",
...
# Result: zone=LOCKED, lane=WARN, auto_approve_allowed=True

# Example 4: Background + LOCKED Zone → STRICT Lane
decision = route(
    trigger="cron",
    actor="CLC",
    path="core/config.yaml",
    op="write",
    context={"wo_id": "WO-20251210-001"}
)
# Result: zone=LOCKED, lane=STRICT, primary_writer=CLC

# Example 5: DANGER Zone → BLOCKED Lane
decision = route(
    trigger="cursor",
    actor="CLS",
    path="/etc/hosts",
...
# Result: zone=DANGER, lane=BLOCKED, primary_writer=None
```
...
- [x] **World Resolution** (Section 1.1) — ✅ Implemented
- [x] **Zone Resolution** (Section 3.2) — ✅ Implemented (DANGER > LOCKED > OPEN)
- [x] **Lane Logic** (Section 5.3) — ✅ Implemented (FAST/WARN/STRICT/BLOCKED)
- [x] **Mission Scope** (Section 2.4) — ✅ Implemented (Whitelist/Blacklist)
- [x] **CLS Auto-approve** (Section 5.3) — ✅ Implemented (5 conditions)
- [x] **Primary Writer** (Section 4.2) — ✅ Implemented
- [x] **Lawset** (Section 5.2) — ✅ Implemented
- [x] **Danger Invariants** (Section 6) — ✅ Implemented (BLOCKED lane)

---

## 📊 Exact Patch Preview (When Boss Approves)

**File to Create:** `bridge/core/router_v5.py`  
**Lines:** ~500 lines  
**Dependencies:** Python 3.8+, `pathlib`, `re`, `json` (stdlib only)

**No breaking changes** — This is a new module, existing `tools/mary_dispatch.py` can coexist.

---

**Status:** ✅ Block 1 Complete — Ready for Review

**Next:** Block 2 (SandboxGuard v5) or proceed with Block 1 implementation?
