...
function run(cmd, env) {
  ...
  try {
    ...
    const t1 = Date.now();
    return { ok:false, ms:t1-t0, err:String(e.message || e).slice(0,100) };
  }
}
...
// Test if sync.cjs exists and is readable
if (!fs.existsSync(SYNC)) {
  console.log('❌ sync.cjs not found at:', SYNC);
  process.exit(1);
}
...
for (const c of cases) {
  console.log(`Testing ${c.name} mode...`);
