...
<style>
  ...
  .secret-warning{background:#f8d7da;color:#721c24;border:1px solid #f5c6cb;padding:8px;border-radius:4px;margin:8px 0}
</style>
...
<div id="secret-warning" class="secret-warning" style="display:none">
  ⚠️ <strong>Secret keys detected:</strong> <span id="secret-keys"></span> — Extra confirmation required
</div>

<div id="diff-preview" class="diff" style="display:none;margin-top:16px"></div>

<script>
const SECRET_PATTERNS = ['TOKEN','SECRET','PASSWORD','PASS','API_KEY','WEBHOOK','PRIVATE_KEY','ACCESS_KEY','CLIENT_SECRET'];

let currentConfig = null;
let currentMode = 'off';

function detectSecretKeys(env) {
  const keys = Object.keys(env || {});
  const touched = [];
  for (const k of keys) {
    const up = k.toUpperCase();
    if (SECRET_PATTERNS.some(p => up.includes(p))) touched.push(k);
  }
  return touched;
}
...
async function previewChanges(){
  ...
  const secrets = detectSecretKeys(currentConfig.env);
  ...
  if (secrets.length > 0) {
    ...
    secretKeys.textContent = secrets.join(', ');
  } else {
...
async function applyChanges(){
  ...
  const secrets = detectSecretKeys(currentConfig.env);
  if (secrets.length > 0) {
    if (!confirm('Secret keys detected: ' + secrets.join(', ') + '\\n\\nAre you sure you want to apply these changes? This will modify sensitive configuration.')) {
      return;
    }
  } else if (!confirm('Are you sure you want to apply these changes? This will modify the .env file.')) {
    return;
  }
  ...
  try{
    ...
    const r = await fetch('/api/config/apply', {
      method: 'POST',
      headers: headers,
      body: JSON.stringify({ env: currentConfig.env, mode: currentMode, confirm: currentMode === 'on', secretConfirm: secrets.length > 0 })
    });
    ...
    if (j.ok) {
      ...
      if (j.details && j.details.secrets) {
        document.getElementById('status').innerHTML += '<div class="status warning">Secret keys blocked: '+j.details.secrets.join(', ')+'</div>';
      }
    }
  }catch(e){
