    ...
    awk '
      BEGIN{inserted=0}
      {
        ...
        if (!inserted && ($0 ~ /^set -e/ || $0 ~ /^set -euo/ || $0 ~ /^set -eu/)) {
          ...
          print "  if [[ -x \"${REPO_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || echo .)}/tools/sync_truth.zsh\" ]]; then"
          print "    zsh \"${REPO_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || echo .)}/tools/sync_truth.zsh\""
          print "  else"
