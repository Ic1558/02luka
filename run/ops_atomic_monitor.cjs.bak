...
if (CI) {
  console.log("[CI mode] Running in GitHub Actions environment");
}
...
async function checkRedis() {
  if (CI) {
    log("[CI mode] Skipping Redis check", "INFO");
    return { status: "pass", message: "Skipped in CI" };
  }
...
async function checkDatabase() {
  if (CI) {
    log("[CI mode] Skipping database check", "INFO");
    return { status: "pass", message: "Skipped in CI" };
  }
...
async function checkLaunchAgents() {
  if (CI) {
    log("[CI mode] Skipping LaunchAgent check", "INFO");
    return { status: "pass", message: "Skipped in CI" };
  }
...
async function main() {
  ...
  try {
    ...
    process.exit(hasErrors ? 1 : 0); // Only fail on critical issues
