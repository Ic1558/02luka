#!/usr/bin/env zsh
set -euo pipefail
MCP_STATUS=${MCP_STATUS:-"8/8 servers up; protocol passes."}
REPO_STATUS=${REPO_STATUS:-"1,289 files; out-of-zone 5 (0.4% within SLA)."}
LATEST_SAVE=${LATEST_SAVE:-"memory/clc/session_251009_141345_note.md"}
LAST_COMMIT=${LAST_COMMIT:-"9ad46d6"}
DOCS_STATUS=${DOCS_STATUS:-"Docs + session summary delivered."}
PROJECT=${PROJECT:-system-stabilization}
TAGS=${TAGS:-ops,menu,starter}

ts=$(date +%y%m%d_%H%M)
out="g/reports/${ts}_ops-menu.md"
mkdir -p g/reports
cat > "$out" <<EOF
---
project: ${PROJECT}
...
EOF
command -v make >/dev/null 2>&1 && make boss-refresh >/dev/null || true
echo "✅ $out"
