...
node -e "const fs = require('fs'); const data = JSON.parse(fs.readFileSync('${tmp_branch_response}', 'utf8')); if (data.error !== 'branch_invalid') { console.error('Expected branch_invalid error'); process.exit(1); } if (!data.details || !Array.isArray(data.details.available_branches)) { console.error('branch_invalid response missing available branches'); process.exit(1); } if (!data.details || data.details.message !== 'Branch is invalid. Branch is either deleted or invalid.') { console.error('branch_invalid response missing expected message'); process.exit(1); }"
