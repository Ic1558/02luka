...
# Default data_json to empty object if not provided
if [[ -z "$DATA_JSON" ]]; then
  DATA_JSON="{}"
fi
...
# Fallback if jq fails - use simple jq compact
if [[ -z "$LEDGER_ENTRY_JSON" ]] || ! echo "$LEDGER_ENTRY_JSON" | jq empty >/dev/null 2>&1; then
  # Build JSON object and compact it
  if [[ -n "$PARENT_ID" ]]; then
    LEDGER_ENTRY_JSON=$(jq -n -c \
      --arg protocol "AP/IO" \
      --arg version "3.1" \
      ...
      --arg summary "$SUMMARY" \
      --argjson data "$DATA_JSON" \
      '{protocol: $protocol, version: $version, ledger_id: $ledger_id, parent_id: $parent_id, ts: $ts, agent: $agent, correlation_id: $correlation_id, session_id: $session_id, event: {type: $event_type, task_id: $task_id, source: $source, summary: $summary}, data: $data, routing: {targets: [$agent], broadcast: false, priority: "normal", delivered_to: []}}' 2>/dev/null || echo "")
  else
    LEDGER_ENTRY_JSON=$(jq -n -c \
      --arg protocol "AP/IO" \
      --arg version "3.1" \
      ...
      --arg summary "$SUMMARY" \
      --argjson data "$DATA_JSON" \
      '{protocol: $protocol, version: $version, ledger_id: $ledger_id, ts: $ts, agent: $agent, correlation_id: $correlation_id, session_id: $session_id, event: {type: $event_type, task_id: $task_id, source: $source, summary: $summary}, data: $data, routing: {targets: [$agent], broadcast: false, priority: "normal", delivered_to: []}}' 2>/dev/null || echo "")
  fi
  
  # Final fallback: manual JSON construction
  if [[ -z "$LEDGER_ENTRY_JSON" ]]; then
    # Escape summary for JSON
    SUMMARY_ESC=$(echo "$SUMMARY" | sed 's/"/\\"/g')
    if [[ -n "$PARENT_ID" ]]; then
      LEDGER_ENTRY_JSON="{\"protocol\":\"AP/IO\",\"version\":\"3.1\",\"ledger_id\":\"$LEDGER_ID\",\"parent_id\":\"$PARENT_ID\",\"ts\":\"$TS\",\"agent\":\"$AGENT\",\"correlation_id\":\"$CORR_ID\",\"session_id\":\"$SESSION_ID\",\"event\":{\"type\":\"$EVENT_TYPE\",\"task_id\":\"$TASK_ID\",\"source\":\"$SOURCE\",\"summary\":\"$SUMMARY_ESC\"},\"data\":$DATA_JSON,\"routing\":{\"targets\":[\"$AGENT\"],\"broadcast\":false,\"priority\":\"normal\",\"delivered_to\":[]}}"
    else
      LEDGER_ENTRY_JSON="{\"protocol\":\"AP/IO\",\"version\":\"3.1\",\"ledger_id\":\"$LEDGER_ID\",\"ts\":\"$TS\",\"agent\":\"$AGENT\",\"correlation_id\":\"$CORR_ID\",\"session_id\":\"$SESSION_ID\",\"event\":{\"type\":\"$EVENT_TYPE\",\"task_id\":\"$TASK_ID\",\"source\":\"$SOURCE\",\"summary\":\"$SUMMARY_ESC\"},\"data\":$DATA_JSON,\"routing\":{\"targets\":[\"$AGENT\"],\"broadcast\":false,\"priority\":\"normal\",\"delivered_to\":[]}}"
    fi
  fi
fi
