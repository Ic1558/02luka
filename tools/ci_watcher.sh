...
if [[ $# -eq 0 ]]; then
  echo "🔍 Auto-detecting failing PRs..."
  ...
  for arg in "$@"; do
    if [[ ! "$arg" =~ ^[0-9]+$ ]]; then
      echo "❌ Invalid PR number: $arg (must be numeric)"
      exit 1
    fi
  done
  PR_NUMBERS=("$@")
...
for pr_num in "${PR_NUMBERS[@]}"; do
  ...
  if [[ -z "$PR_INFO" ]]; then
    echo "❌ PR #$pr_num not found or inaccessible"
  ...
  if [[ -z "$FAILING_CHECKS" ]]; then
    echo "✅ No failing checks found (may have passed since last check)"
  ...
  if [[ -n "$LATEST_REPORT" ]]; then
    ...
    if [[ $ELAPSED_MINUTES -lt $BACKOFF_MINUTES ]]; then
      ...
      echo "⏸️  Skipping PR #$pr_num (rerun $ELAPSED_MINUTES min ago, wait $REMAINING more min)"
  ...
  echo ""
  echo "♻️  Triggering rerun via Puppeteer..."
  
  RERUN_OUTPUT=$(node "${HOME}/02luka/tools/puppeteer/run.mjs" pr-rerun \
    --url "$PR_URL" 2>&1) || {
    echo "⚠️  Puppeteer rerun failed (may need manual rerun)"
  ...
  echo "⏳ Waiting 5 seconds for checks to start..."
  sleep 5
...
"
  ...
  echo "✅ PR #$pr_num processed"
done

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "✅ CI Watcher complete"
