...
# Ensure PATH includes Homebrew (critical for GUI/LaunchAgent environments)
export PATH="/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:$PATH"

# Handle --help flag (must be first, before any other processing)
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<EOF
...
Framework: g/docs/PR_MANAGEMENT_DECISION_FRAMEWORK_v1.md (optional)
EOF
  exit 0
fi
...
if ! gh auth status >/dev/null 2>&1; then
  echo "⚠️  Warning: gh not authenticated. Run: gh auth login" >&2
  echo "   Continuing anyway, but may fail to fetch PR data..." >&2
fi

# Framework is optional (tool can work without it, just won't show reference at end)
# Don't exit if missing - tool logic doesn't require it
...
for pr_num in "${PR_NUMS[@]}"; do
  ...
  # Parse with error handling (gh already outputs JSON, no need for fromjson)
  title=$(echo "$pr_data" | jq -r '.title // "[Title unavailable]"' 2>/dev/null || echo "[Title unavailable]")
  mergeable=$(echo "$pr_data" | jq -r '.mergeable // "UNKNOWN"' 2>/dev/null || echo "UNKNOWN")
  files=$(echo "$pr_data" | jq -r '.files[]?.path // empty' 2>/dev/null || echo "")
  ...
  # Check branch divergence (handle JSON parsing errors)
  commits_data=$(gh pr view "$pr_num" --json commits 2>/dev/null || echo "")
  if [[ -n "$commits_data" ]]; then
    divergence=$(echo "$commits_data" | jq -r '.commits | length // 0' 2>/dev/null || echo "0")
...
if [[ -f "$FRAMEWORK" ]]; then
  ...
  echo "📖 Framework: Not found (optional)"
fi
AUTOPILOT="$LUKA_SOT/g/docs/PR_AUTOPILOT_RULES.md"
if [[ -f "$AUTOPILOT" ]]; then
  echo "🤖 Autopilot Rules: $AUTOPILOT"
fi
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
