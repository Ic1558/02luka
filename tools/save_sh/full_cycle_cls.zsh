...
REPO_ROOT=${SAVE_SH_REPO_ROOT:-$DEFAULT_REPO}
cd "$REPO_ROOT"

TEST_DIR="$REPO_ROOT/logs/save_sh/tests"
mkdir -p "$TEST_DIR"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
TEST_LOG="$TEST_DIR/full_cycle_cls_${TIMESTAMP}.log"

echo "=== save.sh full-cycle test (CLS lane) ==="
...
set +e
set -o pipefail
SAVE_SH_LANE="CLS" LUKA_MLS_AUTO_RECORD=1 bash "$REPO_ROOT/tools/save.sh" | tee "$TEST_LOG"
...
MLS_MARKER="save.sh full-cycle (CLS)"
...
if [[ $TEST_STATUS -eq 0 && "$MLS_RESULT" != "recorded" ]]; then
  echo "⚠️  MLS record not detected for CLS lane" >&2
  TEST_STATUS=5
fi
...
if [[ $TEST_STATUS -eq 0 ]]; then
  echo "✅ CLS full-cycle test passed"
else
  echo "❌ CLS full-cycle test failed (code $TEST_STATUS)"
fi
