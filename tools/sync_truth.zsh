...
# 4.1) Create bridge health stub if missing (Paused by design)
BRIDGE_HEALTH_FILE="$ROOT/g/telemetry/bridge_health.json"
if [[ ! -f "$BRIDGE_HEALTH_FILE" ]]; then
  mkdir -p "$(dirname "$BRIDGE_HEALTH_FILE")"
  cat > "$BRIDGE_HEALTH_FILE" <<EOF
{
  "status": "paused",
  "reason": "no launchagent by design (OPTIONAL)",
  "last_seen": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
  "note": "Stub generated by Truth Sync"
}
EOF
  echo "Created stub: $BRIDGE_HEALTH_FILE"
fi
