...
' "$lc_line" | awk '{print $2}')"
    ...
    channel="$(printf '%s
' "$label" | sed 's/^com[.]02luka[.]//')"
    subs="$(safe_redis_numsub "$channel")"
    [[ -n "$subs" ]] || subs=-1

    agent_status="$(status_for_agent "$pid" "$exit_code" "$subs")"
    ...
    log "checked label=$label pid=$pid exit=$exit_code subs=$subs status=$agent_status"
  done
done
