...
PYYAML_AVAILABLE=1
if [[ "${MARY_FORCE_NO_PYYAML:-0}" == "1" ]]; then
  PYYAML_AVAILABLE=0
elif ! python3 -c 'import yaml' >/dev/null 2>&1; then
  PYYAML_AVAILABLE=0
fi

if (( ! PYYAML_AVAILABLE )); then
  log "⚠️ PyYAML module not available – dispatcher running in fallback mode"
  echo "[mary_dispatcher] WARNING: PyYAML module not available – routing limited to CLC" >&2
fi
...
for file in "$INBOX"/*.yaml "$INBOX"/*.yml; do
  ...
  if [[ "$dest" == "LPE" ]] && (( ! PYYAML_AVAILABLE )); then
    log "$id -> CLC (PyYAML unavailable; cannot convert to LPE JSON)"
    dest="CLC"
  fi
